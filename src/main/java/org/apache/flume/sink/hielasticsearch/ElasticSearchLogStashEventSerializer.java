  ...
  private void appendHeaders(XContentBuilder builder, Event event)
    ...
    if (!StringUtils.isBlank(source)
      ...
      ContentBuilderUtil.appendField(builder, "@source",
          source.getBytes(charset.name()));
    }

    String type = headers.get("type");
    if (!StringUtils.isBlank(type)
        && StringUtils.isBlank(headers.get("@type"))) {
      ContentBuilderUtil.appendField(builder, "@type", type.getBytes(charset.name()));
    }
    ...
    if (!StringUtils.isBlank(srcPath)
      ...
      ContentBuilderUtil.appendField(builder, "@source_path",
          srcPath.getBytes(charset.name()));
    }

    builder.startObject("@fields");
    for (String key : headers.keySet()) {
      byte[] val = headers.get(key).getBytes(charset.name());
